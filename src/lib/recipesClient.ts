export class RecipesClient {
    ...
    private static API_RECIPE_SEARCH_URL = (query: string) =>
        this.API_BASE_URL +
        "/complexSearch?" +
        new URLSearchParams({ query, apiKey: this.API_KEY });
    ...
    static searchRecipes = async (
        ...
        try {
            const response = await fetch(this.API_RECIPE_SEARCH_URL(keyword));
            if (!response.ok) {
